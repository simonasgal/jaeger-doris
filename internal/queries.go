...
func queryGetTrace(schema *SchemaMapping, tableName string, traceID string) string {
	fnq := func(tableName string) string {
		return fmt.Sprintf(
	...
	return fmt.Sprintf("%s UNION %s", fnq(tableName), fnq("otel.otel_traces_v2"))
}
...
func queryFindTraces(schema *SchemaMapping, tableName string, traceIDs []string, partitionTS time.Time) string {
	fnq := func(tableName string) string {
		...
		for i, traceID := range traceIDs {
			traceIDs[i] = fmt.Sprintf("%q", traceID)
		}
	...
	return fmt.Sprintf("((%s) UNION ALL (%s))", fnq(tableName), fnq("otel.otel_traces_v2"))
}
...
func queryFindTraceIDs(schema *SchemaMapping, tableName string, param *spanstore.TraceQueryParameters, location *time.Location) string {
	...
	return fmt.Sprintf("((%s) UNION ALL (%s)) ORDER BY 2 DESC LIMIT %d",
		fnq(tableName), fnq("otel.otel_traces_v2"),
		param.NumTraces,
	)
}
